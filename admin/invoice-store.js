...
class InvoiceStore{
  ...
  async saveDraft(payload){return this.#transaction(()=>this.#service.saveDraft(payload))}
  async issue(payload){return this.#transaction(()=>this.#service.issue(payload))}
  async registerPayment(payload){const snapshot=this.#state;this.#set({saving:true,error:null},cfg.events.saving,{snapshot,operation:'payment'});try{const result=await this.#service.registerPayment(payload);this.#publish('fitconnect:invoice-payment-registered',{payload,result});this.#set({saving:false},null);return result}catch(error){this.#state=snapshot;this.#publish(cfg.events.rollback,{snapshot,error,operation:'payment'});throw error}}
  requestDocument(action,invoice=this.#state.activeInvoice){this.#publish('fitconnect:invoice-document-requested',{action,invoice});return invoice}
  async #transaction(operation){const snapshot=this.#state;this.#set({saving:true,error:null},cfg.events.saving,{snapshot});try{const saved=await operation();const invoices=freeze([...this.#state.invoices.filter(invoice=>invoice.id!==saved.id),saved]);this.#set({invoices,activeInvoice:saved,lines:saved.line_snapshot||freeze([]),saving:false},cfg.events.saved,{invoice:saved});return saved}catch(error){this.#state=snapshot;this.#publish(cfg.events.rollback,{snapshot,error});throw error}}
}
