(()=>{
  ...
  class ProductStore{
    #state;
    constructor(service,config){if(!service)throw new TypeError('ProductService is verplicht.');this.service=service;this.config=config;this.listeners=new Set();this.#state={products:[],selectedProduct:null,loading:false,savingIds:new Set(),lastRefresh:null,version:config.version||'1.0',error:null}}
    ...
    emit(event={type:'state.changed'}){const snapshot=this.getState();this.listeners.forEach(callback=>callback(snapshot,event))}
