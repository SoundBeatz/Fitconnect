...
async function harmonizeRegistryRuntime(){
  ...
  window.__fitConnectRegistryHarmonizationPromise=(async()=>{
    if(!window.FitConnectModuleRegistry){throw new Error('Module Registry runtime ontbreekt.');}
...
function establishLegacyDomContract(){ensureCompatibilityTarget('duplicateProduct');isolateLegacyModuleRegistry();window.__fitConnectLegacyDomContract={ready:true,version:'20260813-auth-loop-repair-v1.0',registryIsolation:true,canonicalRegistry:'#moduleRegistryCanonical',legacySandbox:'#moduleRegistry'}}
