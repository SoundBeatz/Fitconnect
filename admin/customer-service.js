...
class CustomerService{
  ...
  normalize(input){const address=input.address||{},fiscal=input.fiscal||{};return Object.freeze({...input,fullName:clean(input.fullName,cfg.limits.name),companyName:clean(input.companyName,cfg.limits.companyName),contactName:clean(input.contactName,cfg.limits.name),email:clean(input.email,cfg.limits.email).toLowerCase(),phone:clean(input.phone,cfg.limits.phone),address:Object.freeze({line1:clean(address.line1,cfg.limits.addressLine),postalCode:clean(address.postalCode,cfg.limits.postalCode).toUpperCase(),city:clean(address.city,cfg.limits.city),countryCode:clean(address.countryCode||cfg.fiscal.defaultCountryCode,cfg.limits.countryCode).toUpperCase()}),fiscal:Object.freeze({vatNumber:clean(fiscal.vatNumber,cfg.limits.vatNumber).toUpperCase(),kvkNumber:clean(fiscal.kvkNumber,cfg.limits.kvkNumber)})})}
  validate(customer){if(!customer.fullName&&!customer.companyName)throw new Error('Customer name or company name is required');if(customer.email&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(customer.email))throw new Error('Invalid customer email');if(customer.discountPercent!=null&&(Number(customer.discountPercent)<0||Number(customer.discountPercent)>100))throw new Error('Discount must be between 0 and 100');return customer}
  async list(context={}){const args=await this.#run(this.#before,'list',context);const customers=await this.#repository.list(args);return this.#run(this.#after,'list',customers)}
  async save(input,context={}){let customer=this.validate(this.normalize(input));customer=await this.#run(this.#before,'save',customer);const saved=await this.#repository.save(customer,context);return this.#run(this.#after,'save',saved)}
  async saveInvoiceCustomer(input,context){return this.save(input,context)}
