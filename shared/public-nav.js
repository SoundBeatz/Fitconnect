(()=>{
  ...
  (async()=>{
    try{
      if(!window.FITCONNECT_SUPABASE)await addScript(`${root}shared/supabase-config.js?v=20260715-1`,'fc-supabase-config');
  ...
  const loginTile=`<a class="fc-nav-tile ${active==='login'?'active':''}" href="${root}login/">Inloggen</a>`;
  header.className='fc-public-nav';
  header.setAttribute('data-fc-public-nav','');
  header.innerHTML=`<a class="fc-brand" href="${root}"><span class="fc-brand-mark">FC</span><span>FitConnect</span></a><button class="fc-nav-mobile-toggle" type="button" aria-expanded="false" aria-label="Menu openen">☰</button><nav class="fc-nav-grid is-collapsed" aria-label="Hoofdnavigatie"><a class="fc-nav-tile ${active==='home'?'active':''}" href="${root}">Home</a><a class="fc-nav-tile ${active==='shop'?'active':''}" href="${root}shop/">Shop</a><a class="fc-nav-tile ${active==='configurator'?'active':''}" href="${root}configurator/">Gym ontwerp</a><a class="fc-nav-tile fc-nav-muted" href="${root}#expertise">Over FitConnect</a>${cartTile}${loginTile}</nav>`;
