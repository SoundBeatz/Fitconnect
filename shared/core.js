(()=>{
  ...
  function loadStyle(path,id,version){
    return new Promise((resolve,reject)=>{
      let link=document.getElementById(id);const href=withVersion(path,version);
      if(link&&link.href===href)return resolve(link);if(link)link.remove();
      link=document.createElement('link');link.id=id;link.rel='stylesheet';link.href=href;
      link.onload=()=>resolve(link);link.onerror=()=>reject(new Error(`Stylesheet kon niet laden: ${path}`));document.head.appendChild(link);
    });
  }

  function loadScript(path,id,version,{external=false}={}){
    return new Promise((resolve,reject)=>{
      let script=document.getElementById(id);const src=external?path:withVersion(path,version);
      if(script&&script.src===src){if(script.dataset.loaded==='true'||script.readyState==='complete')return resolve(script);script.addEventListener('load',()=>resolve(script),{once:true});script.addEventListener('error',reject,{once:true});return}
      if(script)script.remove();script=document.createElement('script');script.id=id;script.src=src;script.async=false;
      script.onload=()=>{script.dataset.loaded='true';resolve(script)};script.onerror=()=>reject(new Error(`Script kon niet laden: ${path}`));document.head.appendChild(script);
    });
  }

  async function getManifest(){
    const requestUrl=new URL(manifestUrl);requestUrl.searchParams.set('_',Date.now().toString());
    const response=await fetch(requestUrl.href,{cache:'no-store',headers:{'cache-control':'no-cache'}});
    if(!response.ok)throw new Error(`Build manifest niet beschikbaar (${response.status})`);return response.json();
  }
